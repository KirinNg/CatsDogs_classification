...
class Images_test(object):
    def __init__(self, config):
        ...
        self.data = np.zeros([1223,self.config.wide,self.config.wide,3])#
        self.label_0 = np.zeros([1223,37])#
        self.label_1 = np.zeros([1223,2])
        i = 0
        j = 0
        for line_train in train_fin.readlines():
            if j%3==config.choose:
                line_train = line_train.strip().split(' ')
                path = self.config.image_path + line_train[0] + ".jpg"
                img = io.imread(path)
                img = transform.resize(img,(self.config.wide,self.config.wide,3))
...
class Model(object):
    ...
    def _make_model(self):
        def conv(input_data, input_size, output_size, kernel_size = 3, stride = 1):
            w = tf.Variable(tf.random_normal([kernel_size,kernel_size,input_size,output_size],stddev = 0.01))
            b = tf.Variable(tf.constant(0.01, shape = [output_size]))
            return tf.nn.relu(tf.nn.conv2d(input_data, w, [1,stride,stride,1], padding = 'SAME') + b)

        def pooling(input_data):
            return tf.nn.max_pool(input_data, ksize = [1,3,3,1], strides = [1,2,2,1], padding = 'VALID')

        def addLayer(input_data, input_size, output_size):
            W = tf.Variable(tf.random_normal([input_size,output_size],stddev = 0.01))
            basis = tf.Variable(tf.constant(0.01,shape = [output_size]))
    ...
    def _get_feed(self):
        self.min_batch_data = image.data[self.flag*368:self.flag*368+368,:,:,:]
        self.min_batch_label_0 = image.label_0[self.flag*368:self.flag*368+368,:]
        self.min_batch_label_1 = image.label_1[self.flag*368:self.flag*368+368,:]
        self.flag += 1
    ...
    def run_model(self):
        ...
        for i in range(self.config.iter):
            for j in range(10):
                ...
                with tf.device("/cpu:0"):
                    print("i:%d j:%d loss:%.6f accur:%f accur_test:%f" %(i,j,self.sess.run(self.loss, feed),self.sess.run(accuracy, feed),self.sess.run(accuracy, feed_test)))
                    #print("loss:%.6f accur:%f" %(self.sess.run(self.loss, feed),self.sess.run(accuracy, feed)))
                    #print("accur_test:%f" %(self.sess.run(accuracy, feed_test)))
            if i%100 == 0:
                if i>0:
    ...
    def test(self):
        ...
        self.sess.run(tf.global_variables_initializer())
        correct_p = tf.equal(tf.argmax(self.output[:,0:37],1),(tf.argmax(self.label_1,1)))
        accuracy = tf.reduce_mean(tf.cast(correct_p, "float"))
        self.restore_model(self.config.save_path)
        accur = 0
        for i in range(3669):
            ...
            print(i)
        print("accur%f" %(accur/3669))

# MAIN 部分
config = Config()
image = Images(config)
image_test = Images_test(config)
